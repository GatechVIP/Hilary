 ...
 * @param  {boolean}    [opts.isQa]         Whether the discussion is in Q&A form
...
var getDiscussion = module.exports.getDiscussion = function(discussionId, callback) {
    getDiscussionsById([discussionId], null, function(err, discussions) {
        if (err) {
            return callback(err);
        }
 ...
 */
var getDiscussionsById = module.exports.getDiscussionsById = function(discussionIds, fields, callback) {
    ...
    if (fields) {
        ...
        query = 'SELECT ' + placeholders.join(',') + ' FROM Discussions USING CONSISTENCY QUORUM WHERE id IN (?)';
    } else {
        query = 'SELECT * FROM Discussions USING CONSISTENCY QUORUM WHERE id IN (?)';
    }
...
var iterateAll = module.exports.iterateAll = function(properties, batchSize, onEach, callback) {
    ...
    Cassandra.iterateAll(properties, 'Discussions', 'id', opts, _iterateAllOnEach, callback);
};
